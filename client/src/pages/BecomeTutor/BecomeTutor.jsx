...
const BecomeTutor = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setError(err.response?.data?.message || 'Could not save your tutor profile.');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div>
      <span
        style={{ color: '#2563EB', fontWeight: 600, fontSize: '0.875rem', cursor: 'pointer', display: 'inline-block', marginBottom: '1rem' }}
        onClick={() => navigate('/tutors')}
      >
        &larr; Back to Tutor Marketplace
      </span>
      <h1 style={{ fontSize: '1.5rem', marginBottom: '1.5rem' }}>Become a tutor</h1>
      ...
      <form onSubmit={handleSubmit} style={{ maxWidth: '480px' }}>
        <div style={{ marginBottom: '1rem' }}>
          ...
          <textarea
            ...
            style={{
              width: '100%', minHeight: '100px', padding: '0.75rem 1rem', borderRadius: '6px',
        ...
        <Button type="submit" loading={loading}>Save tutor profile</Button>
      </form>
    </div>
  );
};
